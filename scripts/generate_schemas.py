...
def _frozen_schema(entry: _RegistryEntry) -> dict[str, JsonValue]:
    """Return released reviewed bytes unchanged when a newer model version exists."""

    return _load_versioned_template(entry, entry.relative_path)
...
def _event_draft_v1_1_schema(entry: _RegistryEntry) -> dict[str, JsonValue]:
    document = _load_versioned_template(
        ...
        replacements={"opaque-unknown-event-draft-1.0.0": "opaque-unknown-event-draft-1.1.0"},
    )
    definitions = cast(dict[str, JsonValue], document["$defs"])
    branches = cast(list[JsonValue], document["oneOf"])
    for family in (
...
def _opaque_unknown_event_v1_1_schema(entry: _RegistryEntry) -> dict[str, JsonValue]:
    document = _load_versioned_template(
        entry, "events/opaque-unknown-event-draft-1.0.0.schema.json"
    )
    definitions = cast(dict[str, JsonValue], document["$defs"])
    unknown = cast(dict[str, JsonValue], definitions["unknown_event_schema"])
