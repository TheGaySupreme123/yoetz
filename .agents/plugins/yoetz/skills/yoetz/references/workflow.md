...
8. Respond to each challenge by accepting and acting, supplying evidence, revising the claim, disputing with evidence, or stating an unresolved limitation. A readable response identifies the finding as answered and removes it from the unresolved count, but it does not erase the historical finding or close an underlying coverage gap. Repair the record anyway — it stops the next check from minting a duplicate and shows the reader what you decided — and word the receipt according to any remaining coverage limitation.
