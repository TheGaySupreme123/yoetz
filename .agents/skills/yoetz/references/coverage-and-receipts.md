...
`acknowledged`, `provenance_disputed`, `rejected`, and `waived` each record what you decided and what evidence you attached. Use `provenance_disputed` only to contest the finding's authorship or provenance premise; it does not reject the finding's conclusion or resolve the finding. A readable response identifying the finding removes it from the unanswered-finding count without erasing the historical finding. Any independent coverage gap remains coverage-incomplete and can still prevent a clean completion receipt. Word the final answer according to both the unresolved count and the receipt's weakest coverage.
