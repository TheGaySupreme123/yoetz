...
- For a pending **check** with returned session/writer IDs, read `status` with `view=operation` once and replay the same `request_id` once. If it is still pending, continue with a new deterministic-only check and state that the earlier check never reached a terminal result. This fallback cannot recover a first `start` that has not returned IDs.
- For `start`, `BUNDLE_BUSY` with `start_runtime_rebind_retry_ready`, `start_catalog_retry_ready`, or `start_busy_retry_ready` means the service retained the reservation and released its lease. Replay the exact start body and `request_id` once. The runtime waits at most five seconds for an existing same-bundle usage to drain before returning this result; the result does not promise that contention has cleared.
- `OPERATION_PENDING` with `start_lease_pending` means another live lease still owns that start. Wait up to 60 seconds, then replay the exact body and `request_id` once. If it remains pending or busy, retain the request and correlation ID and report the unresolved start. Never mint a new task, guess session/writer IDs, or use a fresh check as start recovery. A lost first-start response likewise requires exact replay, not a status request with invented IDs. Without the retry-ready reason, a busy error makes no claim that a reservation was released.
