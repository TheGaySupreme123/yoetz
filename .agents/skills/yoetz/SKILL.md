...
`respond` records your disposition and links your evidence; it does not clear the finding, whichever disposition you record. Only a later qualifying check of the repaired record resolves it: whole-case or scoped to the finding's subject, owning policy pack run to completion, nothing suppressed, material readable, and for a semantic finding a completed semantic review. The resolved finding stays visible as history (`status view=findings filter.include_resolved=true`) and stops blocking the receipt. Repair the record and recheck; do not call a finding resolved until status or the receipt says so, and if the recheck still fires the issue, do not promise a clean receipt.
