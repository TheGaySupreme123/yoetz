...
- **Answering a finding no longer discards the check that raised it.** `response_recorded` is a
  ...
  frontier it tested rather than the receipt's. Responses to findings the check did not return, and
  every other material event, still require a re-check. Compact status uses the same predicate, so
