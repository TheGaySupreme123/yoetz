...
- For a pending check with returned session/writer IDs, read `status view=operation` once and replay the same request once; if still pending, run a new deterministic-only check and disclose the unresolved earlier check. This is not first-start recovery. For start busy reasons `start_runtime_rebind_retry_ready`, `start_catalog_retry_ready`, or `start_busy_retry_ready`, replay the exact start body and request ID once: the reservation is retained and its lease released. For `start_lease_pending`, wait up to 60 seconds before exact replay. If unresolved, retain the request/correlation and say so; never guess session/writer IDs, create another task, or use a check to recover start. Unclassified busy does not prove lease release.
