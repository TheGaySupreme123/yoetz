...
- A ceiling check whose coverage also carries `optional_semantic_review_registration_drift` records that the last applied install route was `policy`: the serving strict process is stale, so recover with `yoetz integrate codex mcp preview` then `yoetz integrate codex mcp install --route-profile policy`, and start a fresh Codex process. A ceiling gap without the drift gap is a genuinely applied strict route and stays terminal.
