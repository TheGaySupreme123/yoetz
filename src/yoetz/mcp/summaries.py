...
# The claim-revision error message is generated by a closed domain helper, but ordinary MCP
# summaries deliberately do not echo public-error messages.  This exact shape lets the text-only
# host projection recover the registered invariant without trusting arbitrary service/caller text.
_CLAIM_REVISION_MESSAGE: Final = re.compile(
    r"^The event batch is invalid\. claim_recorded/1\.1\.0 correction requires invariant "
    r"(?P<invariant>[a-z][a-z0-9_]{0,63}) at (?P<field>[a-z][a-z0-9_]{0,63})\. "
    r"Use supersedes_claim_refs only for exact prior effective claim ids with overlapping "
    r"obligation scope and a fresh, materially changed claim\. Keep partial or failed result ids "
    r"in limitation_refs rather than supporting_refs, and do not combine replacement with "
    r"disputes_refs\. Read status views candidate_findings, history, and results before retrying\.$",
    re.ASCII,
)
_CLAIM_REVISION_POINTER: Final = re.compile(
    r"^/event_drafts/(?:0|[1-9][0-9]?)/payload/(?P<field>[a-z][a-z0-9_]{0,63})$",
    re.ASCII,
)
...
_CLAIM_REVISION_CORRECTIONS: Final[Mapping[str, str]] = {
    "claim_id_must_be_fresh": "use a fresh claim_id",
    "claim_kind_must_match": "match claim_kind with every superseded claim",
    "limitation_refs_complete": "include every relevant partial or failed result in limitation_refs",
    "limitation_refs_must_be_relevant_non_success_results": (
        "keep only relevant non-success results in limitation_refs"
    ),
    "replacement_must_change_effective_claim": "change the replacement's effective claim meaning",
...
def _claim_revision_clause(error: Mapping[str, JsonValue]) -> str:
    """Render the closed claim-revision invariant without echoing the public error message.

    ``ClaimRevisionMismatch`` carries an invariant in its domain-generated message, while the
    generic MCP projector intentionally does not copy messages.  Recover it only when the whole
    checked-in message shape and the already-normalized field pointer agree; a caller- or service-
    supplied message that differs by one token receives the ordinary reason/field summary.
    """

    details = error.get("safe_details")
    message = error.get("message")
    if not isinstance(details, Mapping) or type(message) is not str:
        return ""
    typed = cast(Mapping[str, JsonValue], details)
    candidate: dict[str, object] = {
    ...
    if gated.get("reason_code") != "claim_revision_mismatch":
        return ""
    pointer = gated.get("field")
    if type(pointer) is not str:
        return ""
    match = _CLAIM_REVISION_MESSAGE.fullmatch(message)
    if match is None:
        return ""
    invariant = match.group("invariant")
    field = match.group("field")
    pointer_match = _CLAIM_REVISION_POINTER.fullmatch(pointer)
    if (
        invariant not in _CLAIM_REVISION_INVARIANTS
        or field not in _CLAIM_REVISION_FIELDS
        or pointer_match is None
        or pointer_match.group("field") != field
    ):
        return ""
    correction = _CLAIM_REVISION_CORRECTIONS.get(invariant)
    if correction is None:
        return ""
    return f" Invariant: {invariant}. Correction: {correction}."
