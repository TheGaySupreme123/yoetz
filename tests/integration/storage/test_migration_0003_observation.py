...
def test_schema_two_upgrade_survives_restart_and_file_copy_restore(tmp_path: Path) -> None:
    ...
    assert report.applied_versions == ("0003", "0004", "0005", "0006")
    expected = {
    ...
    assert reopened.execute("PRAGMA user_version").fetchone() == (6,)
    ...
    assert restored.execute("PRAGMA user_version").fetchone() == (6,)
