...
def test_root_resource_bytes_match_manifest() -> None:
    ...
    assert len(manifest.resources) == 71
...
def test_missing_extra_duplicate_and_traversal_cases_fail(
    ...
    # missing -- dropping the last entry breaks the exact 71-member inventory count.
    ...
    # duplicate -- exactly 71 entries, but one logical_name repeats and one real entry is displaced.
...
def test_public_resource_list_matches_release_artifact() -> None:
    ...
    assert kinds.count("migration") == 2
    ...
    # 52 JSON Schema documents plus the one schema inventory manifest.
