...
def test_cursor_session_start_names_a_missing_admission_entry_only_on_an_authorized_route(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Issue #857: Cursor has no denial event, so sessionStart is its proactive surface.

    The serving-route record and the host's own admission files are read for real; the
    service grant read is replaced so the wiring (host, canonical locator, local-only flag)
    is what this test locks. The grant logic itself is covered in the advisory tests.
    """
    ...
    def session_start(conversation: str) -> str:
        stdout = io.BytesIO()
        assert (
            observe_hooks.handle_cursor_observe(
                event_name="sessionStart",
        ...
        emitted = json.loads(stdout.getvalue())
        assert "hookSpecificOutput" not in emitted
        return str(emitted.get("additional_context", ""))

    # No serving-route record: silent, and no grant read is attempted.
    assert "admission" not in session_start("cursor-admission-unrecorded")
    assert captured == []

    record_serving_route("cursor", "policy", _state=tmp_path)
    context = session_start("cursor-admission-policy")
    assert context.endswith("The owner can run: yoetz integrate cursor admission grant.")
    assert captured == [("cursor", str(tmp_path.resolve()), True)]

    # Both project surfaces carrying the entry reads as present: silent again.
    (tmp_path / ".cursor").mkdir(mode=0o700)
    (tmp_path / ".cursor/permissions.json").write_text(
        json.dumps({"mcpAllowlist": ["yoetz:check"]}), encoding="utf-8"
    )
    (tmp_path / ".cursor/cli.json").write_text(
        json.dumps({"permissions": {"allow": ["Mcp(yoetz:check)"]}}), encoding="utf-8"
    )
    assert "admission" not in session_start("cursor-admission-present")
    assert len(captured) == 1
