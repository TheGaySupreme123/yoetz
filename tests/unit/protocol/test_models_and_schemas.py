...
def test_result_leaf_registry_has_exhaustive_schema_parity() -> None:
    ...
    assert len(derived_patterns) == 667
    ...
    assert len(registry_patterns) == 667
...
def test_schema_catalog_record_shape_and_indexes_are_exact() -> None:
    ...
    assert sum(_count_refs(document.json_schema) for document in catalog.documents) == 1_267
