...
def test_json_version_resource_digest_matches_the_source_manifest(
    ...
    assert len(source_manifest["entries"]) == 81
...
def test_default_json_reports_resource_summary_without_enumerating_entries(
    ...
    assert manifest["resource_counts"]["total"] == "81"


def test_explicit_resources_flag_enumerates_all_81_reviewed_identities(
    installed_candidate: InstalledCandidate,
) -> None:
    """Per spec: explicit ``--resources`` must enumerate exactly the 81 reviewed identities.
