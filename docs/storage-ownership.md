...
| Capture accounting (`capture_backlogs`, `capture_reservations`, `capture_reservation_bootstrap`, `capture_handoff_retirements`, `capture_handoff_retired_count`) | Workspace commitment; task route and capture-ticket identities | Metadata only: counts, bytes, timestamps, and opaque ticket/task identities; the retirement account keeps the last 16 payload-free entries (ticket identity, stage, reason, ticket state, source, task, age, closed quarantine reason) plus a total count | Reservations and inventory proofs change under the interprocess lock and the coordinator's capture lock; retirement accounting commits by ticket identity before tombstoning/releasing; an aged reservation is READY maintenance demand, and unknown or legacy entries are read conservatively or dropped (#695, #836) | stay; tested for bootstrap, reconciliation, and stranded-handoff retirement |
