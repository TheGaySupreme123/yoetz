...
Earlier-check receipt wording is deliberately neutral about the suffix's class. It names the tested
frontier and later accepted records without assuming they are finding responses. Later ingestion
is not later occurrence; asynchronous observation can advance a frontier while the old check remains
attributable. A new check evaluates later material and does not promise to remove every gap.
