...
Native hook artifacts resolve the absolute `yoetz` console executable at render time and record that
path in native marker schema `/2`; a malformed `/2` path invalidates the marker. Portable markers
