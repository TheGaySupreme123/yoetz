...
(`session_superseded` is followed, not quarantined as `ledger_rejected`). Recovery first takes a nonblocking workspace reservation, then holds ordered locks for every eligible ended same-host session through full candidate revalidation, the service RPC, authorized rewrites, and pruning. The revalidation covers unmapped sessions, cross-workspace ownership, mapping identity, and mapping recency; contention or changed state falls back to the ordinary request rather than attaching through a stale selector. A failed attempt records its typed cause (`auto_attach_workspace_unbound`,
