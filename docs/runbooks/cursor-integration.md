...
The official [Cursor hooks reference](https://cursor.com/docs/hooks), checked 2026-09-10,
distinguishes generic post-tool failure from successful tool-result payloads and documents separate
subagent events and product-specific lifecycle support. Recovery therefore uses service-owned
mapping and catalog authority, not a guessed parent/worker completion event or a promise that every
host supplies a final session hook. Existing local IDE/CLI capability boundaries are unchanged.
