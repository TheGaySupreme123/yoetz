...
| `observe status` shows no envelopes for a session | Read `hook_diagnostics.reasons`: `workspace_unresolvable` means the hook's `--workspace` locator could not be canonicalized; `workspace_unconsented` means the session's Git root carries no active consent (a session started in a subdirectory canonicalizes to the same root as the consent, so grant consent at the repository root); `paused` means consent is paused. A `recent_count` of zero means the hooks did not fire. |
| `observe status` exits with `observation_status_failed:<reason>` | The reason names the layer: `workspace_unresolvable` (exit 2) is the locator, `storage_unsafe` (exit 20) is the local state path or a retryable storage fault, `storage_corrupt` (exit 40) is invalid stored data; follow the printed remediation rather than restarting the service. |
