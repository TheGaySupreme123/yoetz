...
| `observe status` shows `undelivered_count` growing while the agent works | Read `oldest_pending_receipt` beside `last_successful_drain`: a recent oldest row under a fresh drain means delivery is keeping pace; an old row under a fresh drain means the backlog is being worked from its head; an old row under a stale drain is stuck and `pending_delivery_causes` names why. Yoetz's own `status`/`receipt`/`read_guidance` calls and every Yoetz pre-event no longer enqueue rows (issue #564), so the growth is ordinary tool traffic or a retryable cause. After the agent stops, `yoetz observe drain --workspace . --json` must end at `terminal: drained`; it repeats passes while rows resolve and never sleeps. |
