...
`--accept` invokes only the fixed `uv tool upgrade yoetz` command after checking that this is the
...
the package outcome unknown. A successful command still reports host refresh and activation as
