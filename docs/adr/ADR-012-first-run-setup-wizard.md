...
1. **`yoetz setup` is a new top-level support sub-app** with `run` (the wizard) and `status`
   ...
   credential or repository privacy setup, host-approved `yoetz consent authorize` may complete the
   prepared action after one retention warning without requiring a second local-terminal ceremony;
