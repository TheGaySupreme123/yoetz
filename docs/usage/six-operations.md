...
| Operation | How often |
| --- | --- |
| `start` | Once per task, before substantive work. A new session's first Yoetz call is `start`. If that call fails without an exact typed continuation, ask for intro and guidance rather than continuing without a task. On resume, attach to the existing task instead of starting a second one. |
