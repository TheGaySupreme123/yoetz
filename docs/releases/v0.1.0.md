...
- **Honest completion receipts:** coverage, provenance, freshness, findings, and limitations remain
  separate. A clean local check is never presented as proof that work is correct.
...
- **Privacy-gated AI-powered review:** optional model review stays behind explicit provider binding,
  reauthenticated policy authority, minimization, secret scanning, and durable egress receipts.
- **Codex-managed ChatGPT subscription route:** the exact Codex npm `0.150.1` macOS-arm64
  app-server cell can own OAuth and model discovery without any token entering Yoetz. It uses a
  dedicated home, no-tool ephemeral turns, explicit setup/disconnect/rollback, and runtime evidence
  that says the upstream OpenAI body is unavailable. Release support remains gated on the exact
  packaged live-evidence matrix.
...
- A configured or logged-in Codex subscription evaluator is not by itself a release capability
  claim. Only the exact packaged two-check and negative-control evidence in the evaluator runbook
  can promote that cell; neighboring Codex versions, platforms, plans, and models do not inherit it.
...
- On Linux, approved external-command checks fail closed in v0.1.0 because the bubblewrap launch
  profile is not release-certified. Built-in local checks and the ledger remain available;
