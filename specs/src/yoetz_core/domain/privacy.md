# src/yoetz_core/domain/privacy.py — privacy policy, disclosure, authorization, and receipt values
...
Define the closed, immutable vocabulary that lets Core decide whether content may reach a network
...
sink even when the client later sends its response elsewhere; Core applies the fence before bytes
leave MCP.
...
cannot be converted into one. Under working gate F-012 it is governed only by the one-attempt
transport-authentication contract; without that founder confirmation, credentialed external
...
A local receipt for `agent_context` records Core's release to the ordinary client boundary, not the
client host's eventual model/provider/retention behavior. A local-model receipt records Core's
