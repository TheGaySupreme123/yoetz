# src/yoetz_core/domain/events.py — the 16 event families, drafts, and accepted envelopes
...
`specs/src/yoetz_core/protocol/schemas.md`, never hand-drifted.
...
- All ID-list fields are tuples, sorted by unsigned ASCII byte order, duplicate-free
  ...
  canonical set profile in `specs/src/yoetz_core/protocol/canonical.md` so
...
|---|---|---:|---|
| `task_title` | str ≤ `MAX_LABEL_BYTES` | yes | User content; lives only in the encrypted payload object owned by `specs/src/yoetz_core/ports/objects.md` |
...
`specs/src/yoetz_core/ports/start_catalog.md`;
violation → `ProtocolValueError("attachment_key_incomplete")`. Authored by `yoetz_engine` as part
of the lifecycle-event phase in `specs/src/yoetz_core/application/start.md`.
...
"current" meant at reattach). As required by `specs/src/yoetz_core/application/start.md`, a resume
...
| `finding_frontier` | Frontier | yes — the full frontier at which the finding was shown, as enforced by `specs/src/yoetz_core/application/respond.md` |
...
| `projection_version` | str (`core/0.1.0`) | yes |
...
| `subject_frontier` | Frontier | yes — the pre-receipt frontier, excluding this event itself, as owned by `specs/src/yoetz_core/application/receipt.md` |
...
type and never strengthens any projection. `specs/src/yoetz_core/protocol/schemas.md` owns the
...
- All validation failures map to public `EVENT_INVALID` at the operation boundary; one invalid
  ...
  `specs/src/yoetz_core/application/publish_work.md`.
