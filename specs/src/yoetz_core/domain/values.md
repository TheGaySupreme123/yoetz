# src/yoetz_core/domain/values.py — frozen domain value types
...
byte-identical determinism contract in ADR-002 and `specs/src/yoetz_core/protocol/canonical.md`
would be unenforceable.

This file contains **no Pydantic, MCP, SQLite, or provider imports**. It may import only the
standard library plus `yoetz_core.protocol.ids` (ID validation), `yoetz_core.protocol.errors`
(`ProtocolValueError`), and `yoetz_core.protocol.coverage` (the `AuthorshipAssurance` enum).
...
deliberately absent from this type; `specs/src/yoetz_core/domain/events.md` owns payload display fields.
