# src/yoetz_core/ports/ledger.py — LedgerPort protocol and its command/result types
...
- `request_digest: str` — `sha256:<hex>` of the publication request identity bytes computed by the
  application using `specs/src/yoetz_core/protocol/canonical.md` (caller logical headers + keyed payload commitments; never
...
Implements the freeze step used by `specs/src/yoetz_core/application/check.md`:
...
Implements the final check commit used by `specs/src/yoetz_core/application/check.md`. In one
