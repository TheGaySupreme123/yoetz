# src/yoetz_core/protocol/canonical.py — restricted-JCS canonicalizer and digest identities
...
- **`entry_digest(envelope)`** — SHA-256 over the canonical bytes of the accepted structural
  `AcceptedEvent` envelope in `specs/src/yoetz_core/domain/events.md` *after* the ledger assigns
