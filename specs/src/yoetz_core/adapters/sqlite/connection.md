# src/yoetz_core/adapters/sqlite/connection.py — verified APSW connection factory and single-writer thread

**Wave:** C | **ADRs:** ADR-001, ADR-003 | **Imports (spec-tree):**
`specs/src/yoetz_core/protocol/errors.md`, `specs/src/yoetz_core/config/paths.md`,
`specs/src/yoetz_core/version.md` | **Imported by:**
`specs/src/yoetz_core/adapters/sqlite/repository.md`,
...
2. `verify_private_local_bundle(path)` — delegated to
   `specs/src/yoetz_core/config/paths.md`: rejects
...
`specs/src/yoetz_core/version.md`) and asserts, in order:
...
- `SQLITE_FULL`, `SQLITE_IOERR`, failed sync: propagate as APSW exceptions from the job; the
  caller never acknowledges success under `specs/src/yoetz_core/ports/ledger.md`. They are not converted to
