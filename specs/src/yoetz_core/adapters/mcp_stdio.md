# src/yoetz_core/adapters/mcp_stdio.py — Yoetz-owned bounded stdio MCP transport
...
`specs/src/yoetz_core/mcp/server.md` and ADR-005. This file is the sole owner of the process's stdin and
stdout while `yoetz-core mcp serve` runs. It enforces the frame-size cap before buffering an
...
2. Creates two pairs of AnyIO memory object streams with **capacity zero** carrying the SDK
   `SessionMessage` values consumed by `specs/src/yoetz_core/mcp/server.md`: inbound
