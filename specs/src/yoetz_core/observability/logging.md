# src/yoetz_core/observability/logging.py — structured allowlisted stderr logging

**Wave:** D | **ADRs:** ADR-005, ADR-007, ADR-008, ADR-009 | **Imports (spec-tree):**
`specs/src/yoetz_core/observability/privacy.md`, `specs/src/yoetz_core/config/models.md` |
**Imported by:** `specs/src/yoetz_core/mcp/server.md`, `specs/src/yoetz_core/cli/app.md`,
`specs/src/yoetz_core/application/service.md`, all adapter specs
...
- `record_unexpected_exception_without_raising(exc: BaseException) -> str` — emits one bounded
  ...
  the exception or raises (name fixed by `specs/src/yoetz_core/cli/exits.md` and
  `specs/src/yoetz_core/mcp/errors.md`).
- `record_fatal_exception_without_raising(exc: BaseException) -> None` — emits the same bounded
  ...
  `specs/src/yoetz_core/mcp/server.md`.
...
- `LogMode.mcp_stdio`: stderr `StreamHandler` only, and additionally:
  - a root-level `logging.Filter` is installed on **every** handler that intercepts records from
    ...
    `keyring`, and any logger not under `yoetz_core.*`) at **all levels including `ERROR` and
    `CRITICAL`** (the transport boundary in `specs/src/yoetz_core/adapters/mcp_stdio.md`).
...
`YOETZ_CORE_LOG_LEVEL`/`--log-level` override the file). Yoetz loggers map 1:1 onto stdlib
...
- SDK/third-party records are sanitized at all levels; filtering is structural, not
  level-based.
- No user-controlled plaintext reaches logs; `specs/src/yoetz_core/observability/privacy.md` and
