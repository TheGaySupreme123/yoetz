# src/yoetz_core/application/maintenance.py — maintenance use-case orchestration and consent gate
...
- `MaintenanceService(maintenance: MaintenancePort, clock: ClockPort,
  diagnostics: DiagnosticsPort)` — frozen injected facade.
- Async methods:
...
- Strict boundary values `BackupRequest`, `RestoreRequest`, and `MigrationRequest`, which convert to
  the corresponding port commands without retaining secrets in repr/logs.
...
Only after exact confirmation, a portable plan invokes the confidential local-human helper and
receives one service-internal `RecoverySecret` bound to request ID, plan digest, service generation,
and `create|restore` operation. Portable backup/create requires two exact local entries and one
transmitted/captured handle; portable restore requires one entry and one handle. Machine-bound plans
never open that channel. The application then calls the matching
...
2. Require confirmation. A target equal to current version returns an explicit no-op plan/result;
   downgrade and skipped versions are rejected.
...
Diagnostics record operation kind, request/task ID, plan/result digests, versions, counts, duration,
and bounded reason only. They exclude locations, manifest contents, object IDs if not needed, key
locator/secret, paths, SQL and exception text.
