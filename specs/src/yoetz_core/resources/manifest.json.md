# src/yoetz_core/resources/manifest.json — installed resource identity manifest
...
- `package_path`: POSIX relative path below `src/yoetz_core/resources/`;
...
within `src/yoetz_core/resources/`. Exactly one entry names every packaged file other than the
...
`yoetz-core version --json` reports the resource-set version and digest. It does not print local
