# src/yoetz_core/config/models.py — strict configuration models and profiles
...
`specs/src/yoetz_core/config/load.md`, `specs/src/yoetz_core/config/paths.md`,
`specs/src/yoetz_core/service/daemon.md`, `specs/src/yoetz_core/application/service.md`
...
|---|---|---|---|
| `strict-local` | Core network denied | Deterministic plus optional separately approved AF_UNIX local model | Default private Core path; “offline” includes the separate runtime only with exact sandbox evidence |
...
- Secrets never live in normal TOML, environment, or CLI/MCP values (ADR-004, ADR-008, ADR-009 and
  `specs/src/yoetz_core/observability/privacy.md`);
  the model
