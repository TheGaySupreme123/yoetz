# src/yoetz/resources/schemas/events/session-resumed-1.0.0.schema.json — installed byte-identical schema copy

**Wave:** A/F | **ADRs:** ADR-002, ADR-005, ADR-007 | **Imports (spec-tree):**
`specs/schemas/events/session-resumed-1.0.0.schema.json.md`,
`specs/src/yoetz/resources/manifest.json.md` | **Imported by:** packaging and runtime schema
verification

## Purpose

Specify the packaged copy of the reviewed session-resumed schema. The installed file mirrors the
reviewed source byte-for-byte.

## Public surface

- Logical resource: `schemas/events/session-resumed-1.0.0.schema.json`.
- Installed package path: `src/yoetz/resources/schemas/events/session-resumed-1.0.0.schema.json`.
