...
- `run_provider_setup(fireworks=False, provider=None, model=None, api_key=None) -> int` — the
  short path used by `yoetz --set`; it starts the local service on demand, performs required vault
  setup/unlock, optionally applies one exact reviewed provider preset and model without asking for
  internal binding identifiers, then enters the credential ceremony. An explicit API-key value is
  never echoed and is converted immediately to the mutable one-shot credential buffer; omission
  uses hidden input.
...
Fireworks, Anthropic Claude, Google Gemini, OpenRouter, Vercel AI Gateway, or an
owner-declared HTTPS origin+model (writes
...
- The short `--set` surface preserves the ADR-012 explicit `--api-key VALUE` compatibility
  exception. It never echoes the value, warns through the existing ADR contract about
  shell-history/process-list exposure, and passes it only through mutable confidential-ceremony
  buffers. Omission always uses hidden local-terminal input; provider selection never changes that
  credential boundary.
