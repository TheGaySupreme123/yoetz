...
`specs/OPEN_QUESTIONS.md` gate `W-C-001` must freeze the complete importer-table DDL and collapse
the currently distributed review fragments into this one standalone byte-exact migration before
Wave C materializes `migrations/bundle/0001.sql`. The already-frozen projection/query sidecars may
not be weakened while closing that gate.
