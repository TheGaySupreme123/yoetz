...
The `p1_projection_state` latest-check columns include the check event/frontier/verdict, canonical
returned-finding-ID tuple, nonnegative `suppressed_count`, and exact coverage; all are jointly null
when no check exists and jointly present otherwise.
