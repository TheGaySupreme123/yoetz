# fixtures/adversarial/ADV-005-legitimate-plan-revision.case.json — legitimate plan revision adversarial case
...
Freeze preservation of honest scope adaptation without punishing a disclosed revision as synthetic, public, deterministic evidence before implementation.

## Public surface

One canonical strict-JSON fixture case with `fixture_schema: "yoetz.fixture-case/1.0.0"`, `fixture_version: "1.0.0"`, `fixture_id: "ADV-005"`, a publication-safe purpose, owning requirement IDs, minimum component versions, deterministic controls, typed input variants, and typed expected assertions.

## Behavior

The `input` section contains original obligation, decision rationale, explicit plan revision, matching completion claim, and dishonest control variant. The `expected` section freezes no abandoned-obligation finding for the honest revision while history and limitations remain visible. Every referenced identifier, timestamp, key, digest, nonce, provider response, and fault point is explicit test data; a test may not replace it with current time, randomness, network state, or host paths. Multi-variant cases evaluate each variant independently and declare the relationship between their outcomes.
