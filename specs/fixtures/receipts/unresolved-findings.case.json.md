# fixtures/receipts/unresolved-findings.case.json — unresolved findings receipt case
...
Freeze receipt behavior when current findings still require action as synthetic, public, deterministic evidence before implementation.

## Public surface

One canonical strict-JSON fixture case with `fixture_schema: "yoetz.fixture-case/1.0.0"`, `fixture_version: "1.0.0"`, `fixture_id: "RCP-003"`, a publication-safe purpose, owning requirement IDs, minimum component versions, deterministic controls, typed input variants, and typed expected assertions.

## Behavior

The `input` section contains current acknowledged, rejected, and unresolved finding states with bound evidence. The `expected` section freezes conclusion unresolved_findings_remain, complete finding summaries, and no verified or complete claim. Every referenced identifier, timestamp, key, digest, nonce, provider response, and fault point is explicit test data; a test may not replace it with current time, randomness, network state, or host paths. Multi-variant cases evaluate each variant independently and declare the relationship between their outcomes.
