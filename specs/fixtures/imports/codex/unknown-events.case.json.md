# fixtures/imports/codex/unknown-events.case.json — unknown Codex events import case
...
Freeze forward-safe handling of source event kinds unknown to the pinned importer as synthetic, public, deterministic evidence before implementation.

## Public surface

One canonical strict-JSON fixture case with `fixture_schema: "yoetz.fixture-case/1.0.0"`, `fixture_version: "1.0.0"`, `fixture_id: "IMP-002"`, a publication-safe purpose, owning requirement IDs, minimum component versions, deterministic controls, typed input variants, and typed expected assertions.

## Behavior

The `input` section contains known records surrounding syntactically valid unknown source records. The `expected` section freezes preserved encrypted source, bounded gap counts, quarantined mappings where required, and no invented event semantics. Every referenced identifier, timestamp, key, digest, nonce, provider response, and fault point is explicit test data; a test may not replace it with current time, randomness, network state, or host paths. Multi-variant cases evaluate each variant independently and declare the relationship between their outcomes.
