# fixtures/canonical/accepted-entry-identity.case.json — accepted entry identity vectors
...
Freeze the accepted-entry digest chain over assigned sequence, writer predecessor, and canonical event content as synthetic, public, deterministic evidence before implementation.

## Public surface

One canonical strict-JSON fixture case with `fixture_schema: "yoetz.fixture-case/1.0.0"`, `fixture_version: "1.0.0"`, `fixture_id: "CAN-007"`, a publication-safe purpose, owning requirement IDs, minimum component versions, deterministic controls, typed input variants, and typed expected assertions.

## Behavior

The `input` section contains a fixed prior frontier plus event drafts accepted in two explicit writer-chain arrangements. The `expected` section freezes exact entry digests, writer heads, and total frontiers; request digest is never reused as entry digest. Every referenced identifier, timestamp, key, digest, nonce, provider response, and fault point is explicit test data; a test may not replace it with current time, randomness, network state, or host paths. Multi-variant cases evaluate each variant independently and declare the relationship between their outcomes.
