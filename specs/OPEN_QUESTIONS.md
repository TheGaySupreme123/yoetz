...
|---|---|---|
| P0-1 finding kind/origin and fixture ownership | `FindingKind` and `FindingOrigin` are independent; the exhaustive fifteen-kind trait table fixes priority/actionability, and the reviewed 49-file corpus covers every kind without inventing a `fixtures/policies/` family. | `INTERFACES.md`, finding/policy/fixture owner specs and conformance specs |
